...
func (m *Manager) OpenEditor(ctx context.Context, identifier, editorOverride string, io ExecIO) (int, error) {
	...
	if editor == "" {
		editor, err = m.cfg.Default(ctx, "gwr.editor.default", "GTR_EDITOR_DEFAULT", "none", "defaults.editor")
		if err != nil {
			return 1, err
		}
	}
...
func (m *Manager) RunAI(ctx context.Context, identifier, toolOverride string, args []string, io ExecIO) (int, error) {
	...
	tool := toolOverride
	if tool == "" {
		tool, err = m.cfg.Default(ctx, "gwr.ai.default", "GTR_AI_DEFAULT", "none", "defaults.ai")
		if err != nil {
			return 1, err
		}
	}
