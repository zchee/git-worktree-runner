...
func (m *Manager) CreateWorktree(ctx context.Context, branch string, opts CreateWorktreeOptions) (Target, error) {
	...
	lockPath := filepath.Join(m.repoCtx.CommonDir, "gwr.lock")
...
func (m *Manager) copyIntoWorktree(ctx context.Context, worktreePath string) error {
	includes, err := m.cfg.All(ctx, "gwr.copy.include", "copy.include")
	if err != nil {
		return err
	}
	...
	excludes, err := m.cfg.All(ctx, "gwr.copy.exclude", "copy.exclude")
	if err != nil {
		return err
	}
	...
	includeDirs, err := m.cfg.All(ctx, "gwr.copy.includeDirs", "copy.includeDirs")
	if err != nil {
		return err
	}
	excludeDirs, err := m.cfg.All(ctx, "gwr.copy.excludeDirs", "copy.excludeDirs")
	if err != nil {
		return err
	}
...
func (m *Manager) runHooks(ctx context.Context, phase, dir string, env map[string]string) error {
	values, err := m.cfg.All(ctx, "gwr.hook."+phase, "hooks."+phase)
	if err != nil {
		return err
	}
	if len(values) == 0 {
...
func (m *Manager) resolveDefaultBranch(ctx context.Context) (string, error) {
	configured, err := m.cfg.Default(ctx, "gwr.defaultBranch", "GTR_DEFAULT_BRANCH", "auto", "")
	if err != nil {
		return "", err
	}
	if configured != "auto" {
