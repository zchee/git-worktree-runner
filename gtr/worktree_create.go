...
func (m *Manager) copyIntoWorktree(ctx context.Context, worktreePath string) error {
	includes, err := m.cfg.All(ctx, "gtr.copy.include", "copy.include")
	if err != nil {
		return err
	}
	...
	includeDirs, err := m.cfg.All(ctx, "gtr.copy.includeDirs", "copy.includeDirs")
	if err != nil {
		return err
	}
	excludeDirs, err := m.cfg.All(ctx, "gtr.copy.excludeDirs", "copy.excludeDirs")
	if err != nil {
		return err
	}
...
func (m *Manager) runHooks(ctx context.Context, phase, dir string, env map[string]string) error {
	values, err := m.cfg.All(ctx, "gtr.hook."+phase, "hooks."+phase)
	if err != nil {
		return err
	}
	if len(values) == 0 {
...
func (m *Manager) resolveDefaultBranch(ctx context.Context) (string, error) {
	configured, err := m.cfg.Default(ctx, "gtr.defaultBranch", "GTR_DEFAULT_BRANCH", "auto", "")
	if err != nil {
		return "", err
	}
	if configured != "auto" {
