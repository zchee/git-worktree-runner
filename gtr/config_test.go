...
func TestManagerConfigSetGetAddUnset(t *testing.T) {
	...
	if err := m.ConfigSet(t.Context(), "gtr.worktrees.prefix", "wt-", false); err != nil {
		t.Fatalf("ConfigSet() error: %v", err)
	}

	values, err := m.ConfigGet(t.Context(), "gtr.worktrees.prefix", false)
	if err != nil {
	...
	if err := m.ConfigAdd(t.Context(), "gtr.copy.include", ".env.local", false); err != nil {
		t.Fatalf("ConfigAdd() error: %v", err)
	}
	if err := m.ConfigAdd(t.Context(), "gtr.copy.include", ".env.example", false); err != nil {
		t.Fatalf("ConfigAdd() error: %v", err)
	}

	values, err = m.ConfigGet(t.Context(), "gtr.copy.include", false)
	if err != nil {
		t.Fatalf("ConfigGet() error: %v", err)
	}
	...
	if err := m.ConfigUnset(t.Context(), "gtr.copy.include", false); err != nil {
