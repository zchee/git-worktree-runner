...
func TestOpenEditorCustomCommand(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.editor.default", "/usr/bin/true"); err != nil {
		t.Fatalf("git config gwr.editor.default: %v", err)
	}
...
func TestRunAINoneConfigured(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.ai.default", "none"); err != nil {
		t.Fatalf("git config gwr.ai.default: %v", err)
	}
...
func TestRunAICustomCommand(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.ai.default", "/usr/bin/true"); err != nil {
		t.Fatalf("git config gwr.ai.default: %v", err)
	}
