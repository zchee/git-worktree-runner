...
func TestCreateWorktreeCopiesIncludedFiles(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gtr.copy.include", ".env.local"); err != nil {
		t.Fatalf("git config --add gtr.copy.include: %v", err)
	}
...
func TestHooksPostCreateAndPostRemove(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gtr.hook.postCreate", postCreate); err != nil {
		t.Fatalf("git config --add gtr.hook.postCreate: %v", err)
	}
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gtr.hook.postRemove", postRemove); err != nil {
		t.Fatalf("git config --add gtr.hook.postRemove: %v", err)
	}
