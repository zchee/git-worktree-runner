...
func TestCreateWorktreeCopiesIncludedFiles(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gwr.copy.include", ".env.local"); err != nil {
		t.Fatalf("git config --add gwr.copy.include: %v", err)
	}
...
func TestHooksPostCreateAndPostRemove(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gwr.hook.postCreate", postCreate); err != nil {
		t.Fatalf("git config --add gwr.hook.postCreate: %v", err)
	}
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gwr.hook.postRemove", postRemove); err != nil {
		t.Fatalf("git config --add gwr.hook.postRemove: %v", err)
	}
