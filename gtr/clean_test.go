...
func TestCleanRemovesEmptyDirectories(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.worktrees.dir", ".worktrees"); err != nil {
		t.Fatalf("git config gwr.worktrees.dir: %v", err)
	}
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.worktrees.prefix", "wt-"); err != nil {
		t.Fatalf("git config gwr.worktrees.prefix: %v", err)
	}
