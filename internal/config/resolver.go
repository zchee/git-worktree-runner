...
// Resolver resolves configuration from git config scopes, .gwrconfig, and environment variables.
...
func (r Resolver) gwrconfigPath() string {
	return filepath.Join(r.MainRoot, ".gwrconfig")
}
...
// local git config > .gwrconfig > global git config > system git config > env > fallback.
//
// fileKey is the key name used in .gwrconfig (for example "defaults.editor" for "gwr.editor.default").
func (r Resolver) Default(ctx context.Context, key, envName, fallback, fileKey string) (string, error) {
	...
	if fileKey != "" {
		fv, err := r.getFile(ctx, r.gwrconfigPath(), fileKey)
		if err != nil {
			return "", fmt.Errorf("read .gwrconfig %s: %w", fileKey, err)
		}
		if fv != "" {
			return fv, nil
		}
...
// local git config > .gwrconfig > global git config > system git config.
//
// fileKey is the key name used in .gwrconfig (for example "copy.include" for "gwr.copy.include").
func (r Resolver) All(ctx context.Context, key, fileKey string) ([]string, error) {
	...
	if fileKey != "" {
		fileVals, err := r.getAllFile(ctx, r.gwrconfigPath(), fileKey)
		if err != nil {
			return nil, fmt.Errorf("read .gwrconfig %s: %w", fileKey, err)
		}
