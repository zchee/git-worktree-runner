...
func TestResolverDefault(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.globalValue != "" {
				if _, err := g.Run(t.Context(), repoDir, "config", "--global", "gtr.editor.default", tc.globalValue); err != nil {
			...
			if tc.fileValue != "" {
				...
				if err := os.WriteFile(filepath.Join(repoDir, ".gtrconfig"), content, 0o644); err != nil {
					t.Fatalf("WriteFile(.gtrconfig): %v", err)
				}
			}

			if tc.localValue != "" {
				if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gtr.editor.default", tc.localValue); err != nil {
			...
			got, err := r.Default(t.Context(), "gtr.editor.default", "GTR_EDITOR_DEFAULT", "none", "defaults.editor")
			if err != nil {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for _, v := range tc.local {
				if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gtr.copy.include", v); err != nil {
					t.Fatalf("git config --local --add: %v", err)
				}
			}
			for _, v := range tc.global {
				if _, err := g.Run(t.Context(), repoDir, "config", "--global", "--add", "gtr.copy.include", v); err != nil {
					t.Fatalf("git config --global --add: %v", err)
				}
			}
			for _, v := range tc.system {
				if _, err := g.Run(t.Context(), repoDir, "config", "--system", "--add", "gtr.copy.include", v); err != nil {
			...
			if len(tc.file) > 0 {
				...
				if err := os.WriteFile(filepath.Join(repoDir, ".gtrconfig"), b, 0o644); err != nil {
					t.Fatalf("WriteFile(.gtrconfig): %v", err)
				}
			}
			...
			got, err := r.All(t.Context(), "gtr.copy.include", "copy.include")
			if err != nil {
