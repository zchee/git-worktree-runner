...
func TestResolverDefault(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.globalValue != "" {
				if _, err := g.Run(t.Context(), repoDir, "config", "--global", "gwr.editor.default", tc.globalValue); err != nil {
			...
			if tc.fileValue != "" {
				...
				if err := os.WriteFile(filepath.Join(repoDir, ".gwrconfig"), content, 0o644); err != nil {
					t.Fatalf("WriteFile(.gwrconfig): %v", err)
				}
			}

			if tc.localValue != "" {
				if _, err := g.Run(t.Context(), repoDir, "config", "--local", "gwr.editor.default", tc.localValue); err != nil {
			...
			got, err := r.Default(t.Context(), "gwr.editor.default", "GTR_EDITOR_DEFAULT", "none", "defaults.editor")
			if err != nil {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for _, v := range tc.local {
				if _, err := g.Run(t.Context(), repoDir, "config", "--local", "--add", "gwr.copy.include", v); err != nil {
					t.Fatalf("git config --local --add: %v", err)
				}
			}
			for _, v := range tc.global {
				if _, err := g.Run(t.Context(), repoDir, "config", "--global", "--add", "gwr.copy.include", v); err != nil {
					t.Fatalf("git config --global --add: %v", err)
				}
			}
			for _, v := range tc.system {
				if _, err := g.Run(t.Context(), repoDir, "config", "--system", "--add", "gwr.copy.include", v); err != nil {
			...
			if len(tc.file) > 0 {
				...
				if err := os.WriteFile(filepath.Join(repoDir, ".gwrconfig"), b, 0o644); err != nil {
					t.Fatalf("WriteFile(.gwrconfig): %v", err)
				}
			}
			...
			got, err := r.All(t.Context(), "gwr.copy.include", "copy.include")
			if err != nil {
