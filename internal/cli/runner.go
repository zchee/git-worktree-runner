...
import (
	...
	"github.com/zchee/git-worktree-runner/gtr"
...
// Runner executes the `git-gtr` CLI.
type Runner struct {
	Stdin  io.Reader
	Stdout io.Writer
	Stderr io.Writer
...
func (r Runner) Run(ctx context.Context, args []string) int {
	...
	if err := cmd.ExecuteContext(ctx); err == nil {
		...
		if unknownCmd, ok := parseUnknownCommand(err); ok {
			...
			fmt.Fprint(r.Stderr, "Use 'git gtr help' for available commands\n")
			return exitUsage
		}
...
func (r Runner) newManager(ctx context.Context) (*gtr.Manager, error) {
	m, err := gtr.NewManager(ctx, gtr.ManagerOptions{})
	if err != nil {
		return nil, err
	}
	return m, nil
...
func (r Runner) runList(ctx context.Context, args []string) int {
	...
	fmt.Fprintln(r.Stdout)
	fmt.Fprintln(r.Stdout, "Tip: Use 'git gtr list --porcelain' for machine-readable output")
	return exitSuccess
}
...
func (r Runner) runNew(ctx context.Context, args []string) int {
	...
	for i := 0; i < len(args); {
		...
		default:
			...
			if branch != "" {
				fmt.Fprintln(r.Stderr, "[x] Usage: git gtr new <branch> [options]")
				return exitUsage
			}
			branch = args[i]
			i++
	...
	target, err := m.CreateWorktree(ctx, branch, gtr.CreateWorktreeOptions{
		FromRef:     fromRef,
		FromCurrent: fromCurrent,
		TrackMode:   gtr.TrackMode(trackMode),
		NoCopy:      noCopy,
		NoFetch:     noFetch,
		Force:       force,
...
func (r Runner) runRemove(ctx context.Context, args []string) int {
	...
	if len(idents) == 0 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr rm <id|branch|worktree-name> [<id|branch|worktree-name>...]")
		return exitUsage
	}
	...
	opts := gtr.RemoveWorktreeOptions{
...
func (r Runner) runCopy(ctx context.Context, args []string) int {
	...
	if !allMode && len(targets) == 0 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr copy <target>... [-n] [-a] [--from <source>] [-- <pattern>...]")
		return exitUsage
	}
	...
	results, err := m.Copy(ctx, targets, gtr.CopyOptions{
		From:          source,
		All:           allMode,
		DryRun:        dryRun,
		Patterns:      patterns,
...
func (r Runner) runGo(ctx context.Context, args []string) int {
	if len(args) != 1 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr go <id|branch|worktree-name>")
		return exitUsage
	}
...
func (r Runner) runConfig(ctx context.Context, args []string) int {
	...
	case "get":
		if key == "" {
			fmt.Fprintln(r.Stderr, "[x] Usage: git gtr config get <key> [--global]")
			return exitUsage
		}
	...
	case "set":
		if key == "" || value == "" {
			fmt.Fprintln(r.Stderr, "[x] Usage: git gtr config set <key> <value> [--global]")
			return exitUsage
		}
	...
	case "add":
		if key == "" || value == "" {
			fmt.Fprintln(r.Stderr, "[x] Usage: git gtr config add <key> <value> [--global]")
			return exitUsage
		}
	...
	case "unset":
		if key == "" {
			fmt.Fprintln(r.Stderr, "[x] Usage: git gtr config unset <key> [--global]")
			return exitUsage
		}
...
func (r Runner) runEditor(ctx context.Context, args []string) int {
	...
	for i := 0; i < len(args); {
		...
		default:
			...
			if identifier != "" {
				fmt.Fprintln(r.Stderr, "[x] Usage: git gtr editor <id|branch|worktree-name> [--editor <name>]")
				return exitUsage
			}
			identifier = args[i]
			i++
	...
	exitCode, err := m.OpenEditor(ctx, identifier, editor, gtr.ExecIO{
		Stdin:  r.Stdin,
		Stdout: r.Stdout,
		Stderr: r.Stderr,
	})
...
func (r Runner) runAI(ctx context.Context, args []string) int {
	...
	for i := 0; i < len(args); {
		...
		default:
			...
			if identifier != "" {
				fmt.Fprintln(r.Stderr, "[x] Usage: git gtr ai <id|branch|worktree-name> [--ai <name>] [-- args...]")
				return exitUsage
			}
			identifier = args[i]
			i++
	...
	exitCode, err := m.RunAI(ctx, identifier, tool, toolArgs, gtr.ExecIO{
		Stdin:  r.Stdin,
		Stdout: r.Stdout,
		Stderr: r.Stderr,
	})
...
func (r Runner) runClean(ctx context.Context, args []string) int {
	if len(args) != 0 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr clean")
		return exitUsage
	}
...
func (r Runner) runDoctor(ctx context.Context, args []string) int {
	if len(args) != 0 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr doctor")
		return exitUsage
	}
	...
	gtr.WriteDoctorReport(r.Stdout, report)
	return exitSuccess
}

func (r Runner) runAdapters(ctx context.Context, args []string) int {
	if len(args) != 0 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr adapter")
		return exitUsage
	}
...
func (r Runner) runRun(ctx context.Context, args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(r.Stderr, "[x] Usage: git gtr run <id|branch|worktree-name> <command...>")
		return exitUsage
	}
	...
	exitCode, err := m.Run(ctx, identifier, command, gtr.RunOptions{
		IO: gtr.ExecIO{
			Stdin:  r.Stdin,
			Stdout: r.Stdout,
			Stderr: r.Stderr,
