...
//
// Precedence for BaseDir: git config gwr.worktrees.dir > env GTR_WORKTREES_DIR > default (<parent>/<repo>-worktrees).
// Precedence for Prefix: git config gwr.worktrees.prefix > env GTR_WORKTREES_PREFIX > default ("").
func ResolvePaths(ctx context.Context, cfg config.Resolver) (Paths, error) {
	prefix, err := cfg.Default(ctx, "gwr.worktrees.prefix", "GTR_WORKTREES_PREFIX", "", "")
	if err != nil {
		return Paths{}, fmt.Errorf("resolve gwr.worktrees.prefix: %w", err)
	}

	baseDir, err := cfg.Default(ctx, "gwr.worktrees.dir", "GTR_WORKTREES_DIR", "", "")
	if err != nil {
		return Paths{}, fmt.Errorf("resolve gwr.worktrees.dir: %w", err)
	}
