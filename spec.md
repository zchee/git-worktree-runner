# Go port spec: `git wr` (Git Worktree Runner)

This repository is a Go port of `~/src/github.com/coderabbitai/git-worktree-runner`.

Primary goal: keep the **user-facing CLI** compatible with upstream `git wr`, while providing an idiomatic **Go library API** with roughly the same surface area as the upstream Bash “library” (`lib/*.sh`).
...
- **Config-over-flags:** defaults are set once (via git config and/or `.gwrconfig`), commands stay simple.
...
- Binary name: `git-gwr` (so users run it as `git wr ...`).
- Optional convenience binary: `gwr` (may be provided, but upstream stopped doing this due to `coreutils gwr` conflicts; keep it optional).
...
- **Repository context:** resolve paths/config from the repository’s **common git dir** (so running `git wr` inside an existing linked worktree still targets the same repository-wide worktree set).
- **Worktree folder naming:**
  - default folder name: `sanitize(branch)`
  - if `--name <suffix>`: `sanitize(branch) + "-" + suffix`
  - optional prefix config: `gwr.worktrees.prefix` prepended to folder name.

### `git wr new <branch> [options]`
...
- If no patterns provided, use merged patterns from:
  - `gwr.copy.include` + `.worktreeinclude` file.
- Exclusions are always applied from:
  - `gwr.copy.exclude`.

### `git wr list [--porcelain]`
...
- **Stdout** is reserved for machine-consumable outputs:
  - `git wr go` prints only the path to stdout.
  - `git wr list --porcelain` prints only porcelain lines to stdout.
...
- `gwr.worktrees.prefix`: prefix added to each worktree folder.
- `gwr.defaultBranch`: default branch for new branches (`auto|main|master|<branch>`).
- `gwr.editor.default`: default editor adapter name or `none`.
- `gwr.ai.default`: default AI adapter name or `none`.
- `gwr.copy.include` (multi): file globs to copy on create/copy.
- `gwr.copy.exclude` (multi): file globs to exclude.
- `gwr.copy.includeDirs` (multi): directory names to copy entirely (e.g., `node_modules`).
- `gwr.copy.excludeDirs` (multi): excluded directory patterns (e.g., `node_modules/.cache`, `*/.npm`).
- `gwr.hook.postCreate` (multi): commands executed after worktree creation.
- `gwr.hook.postRemove` (multi): commands executed after worktree removal.
...
### `.gwrconfig` and `.worktreeinclude`
- `.gwrconfig` lives in the **main worktree root** (i.e., the parent of the repository common dir) and uses gitconfig syntax. It is intended to be committed for team defaults.
- `.worktreeinclude` is also read from the **main worktree root** and uses `.gitignore`-style patterns for files to copy (merged into `gwr.copy.include`).

### Precedence (highest → lowest)
Match upstream semantics as closely as practical:
1. local git config (`.git/config`)
2. `.gwrconfig` (repo root)
...
Target import path: `github.com/zchee/git-worktree-runner/gwr`
...
// CreateWorktree creates a new linked worktree for branch (CLI: `git wr new`).
...
Special case:
- `git wr run ...` exits with the executed command’s exit code (and only uses `1/2` for its own argument/target resolution failures).
...
```
cmd/
  git-gwr/              # main CLI entry (git subcommand)
  gwr/                  # optional convenience wrapper (maybe)
gwr/                    # public library API
internal/
  cli/                  # command parsing, help text, porcelain formatting
  config/               # config resolution: git config + .gwrconfig + env
...
Guiding rule: CLI is thin; business logic lives in `gwr/` + `internal/*`.
...
Even when we delegate to the system `git` (Strategy A), we still need **process-level serialization** for `gwr` operations that mutate:
...
Lock file location:
- Prefer the repo’s *common* git dir (worktree-safe): `<git-common-dir>/gwr.lock`
...
- If lock acquisition times out (default, e.g., 30s), return a clear error:
  - “Another git wr operation is in progress for this repository.”
...
- Use go-git for:
  ...
  - parsing `.gwrconfig` (gitconfig syntax) where helpful.
- Use `os/exec` for:
...
- Always open repos with `git.PlainOpenWithOptions(..., DetectDotGit=true, EnableDotGitCommonDir=true)` so running `git wr` *inside an existing worktree* still resolves the main repo’s common directory correctly.
...
9. Implement base dir resolution (`gwr.worktrees.dir` / env / default sibling).
...
2. **Config precedence:** Should we support global/system gitconfig with full include semantics (likely requires shelling out to `git config`), or is “local + .gwrconfig + env” acceptable for v1?
