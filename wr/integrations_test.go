...
func TestOpenEditorCustomCommand(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "wr.editor.default", "/usr/bin/true"); err != nil {
		t.Fatalf("git config wr.editor.default: %v", err)
	}
...
func TestRunAINoneConfigured(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "wr.ai.default", "none"); err != nil {
		t.Fatalf("git config wr.ai.default: %v", err)
	}
...
func TestRunAICustomCommand(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "wr.ai.default", "/usr/bin/true"); err != nil {
		t.Fatalf("git config wr.ai.default: %v", err)
	}
