...
func TestCleanRemovesEmptyDirectories(t *testing.T) {
	...
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "wr.worktrees.dir", ".worktrees"); err != nil {
		t.Fatalf("git config wr.worktrees.dir: %v", err)
	}
	if _, err := g.Run(t.Context(), repoDir, "config", "--local", "wr.worktrees.prefix", "wt-"); err != nil {
		t.Fatalf("git config wr.worktrees.prefix: %v", err)
	}
