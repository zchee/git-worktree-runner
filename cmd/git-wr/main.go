...
import (
	...
	"github.com/zchee/git-worktree-runner/gwr"
...
// Runner executes the `git-gwr` CLI.
type Runner struct {
	Stdin  io.Reader
	Stdout io.Writer
	Stderr io.Writer
...
func (r Runner) newManager(ctx context.Context) (*gwr.Manager, error) {
	m, err := gwr.NewManager(ctx, gwr.ManagerOptions{})
	if err != nil {
		return nil, err
	}
	return m, nil
...
func (r Runner) runNew(ctx context.Context, args []string) int {
	...
	target, err := m.CreateWorktree(ctx, branch, gwr.CreateWorktreeOptions{
		FromRef:     fromRef,
		FromCurrent: fromCurrent,
		TrackMode:   gwr.TrackMode(trackMode),
		NoCopy:      noCopy,
		NoFetch:     noFetch,
		Force:       force,
...
func (r Runner) runRemove(ctx context.Context, args []string) int {
	...
	opts := gwr.RemoveWorktreeOptions{
...
func (r Runner) runCopy(ctx context.Context, args []string) int {
	...
	results, err := m.Copy(ctx, targets, gwr.CopyOptions{
		From:          source,
		All:           allMode,
		DryRun:        dryRun,
		Patterns:      patterns,
...
func (r Runner) runEditor(ctx context.Context, args []string) int {
	...
	exitCode, err := m.OpenEditor(ctx, identifier, editor, gwr.ExecIO{
		Stdin:  r.Stdin,
		Stdout: r.Stdout,
		Stderr: r.Stderr,
	})
...
func (r Runner) runAI(ctx context.Context, args []string) int {
	...
	exitCode, err := m.RunAI(ctx, identifier, tool, toolArgs, gwr.ExecIO{
		Stdin:  r.Stdin,
		Stdout: r.Stdout,
		Stderr: r.Stderr,
	})
...
func (r Runner) runDoctor(ctx context.Context, args []string) int {
	...
	gwr.WriteDoctorReport(r.Stdout, report)
	return exitSuccess
}
...
func (r Runner) runRun(ctx context.Context, args []string) int {
	...
	exitCode, err := m.Run(ctx, identifier, command, gwr.RunOptions{
		IO: gwr.ExecIO{
			Stdin:  r.Stdin,
			Stdout: r.Stdout,
			Stderr: r.Stderr,
